...
drawstatusbar(Monitor *m, int bh, char* stext) {
	...
	ret = x = m->ww - w;
	x = ret - getsystraywidth(); //to fix statusbar and systray overlapping
		...
		if (text[i] == '^' && !isCode) {
			...
			while (text[++i] != '^') {
				if (text[i] == 'c') {
					...
					i += 7;
					//added for SchemeStatusText --start
					/*
					...
					*/
					//added for SchemeStatusText --end
				} else if (text[i] == 'b') {
					char buf[8];
					memcpy(buf, (char*)text+i+1, 7);
					buf[7] = '\0';
...
//		drw_text(drw, m->ww - tw - stw, 0, tw, bh, lrpad / 2 - 2, stext, 0);
		// 绘制STATUSBAR
		status_w = m->ww - drawstatusbar(m, bh, stext);
...
//	if ((w = m->ww - tw - stw - x) > bh) {
	//system_w == stw
	//status_w == tw
	if ((w = m->ww - status_w - system_w - x) > bh) {
		if (m->sel) {
...
/*change XFillRectangle cause conflict between systray and alpha*/
